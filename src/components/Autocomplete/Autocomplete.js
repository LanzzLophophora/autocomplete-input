...
class Autocomplete extends Component {
  ...
  handleOptionHover = (optionsLength, hoveredOptionIndex, down) => {
    if (!this.autocompleteSelectorRef || !this.autocompleteSelectorRef.scrollHeight) {
      return null;
    }

    const { optionsHeight, maxVisibleOptions } = this.state;
    const firstVisibleOptionPosition = this.autocompleteSelectorRef.scrollTop / optionsHeight;
    const lastVisibleOptionPosition = firstVisibleOptionPosition + maxVisibleOptions - 1;

    let newHoveredOptionIndex = hoveredOptionIndex;
    if (down) {
      if (optionsLength - 1 > newHoveredOptionIndex && newHoveredOptionIndex !== null) {
  ...
  getFilteredOptions = () => {
    ...
    return options
      .filter(opt => opt.substr(0, value.length).toLowerCase() === value.toLowerCase())
      .sort();
  };

  renderOptions = filteredOptions => {
    const { value, visible, hoveredOptionIndex, optionsHeight, maxVisibleOptions } = this.state;
    if (!value || !visible || !filteredOptions || !filteredOptions.length) return null;

    const { style = {}, classNames = {} } = this.props;

    const maxHeight = maxVisibleOptions * optionsHeight;
    const selectorStyles = { ...style.autocompleteSelector, maxHeight: maxHeight + 'px' };
    const optionsStyles = { ...style.autocompleteSelectorOptions, height: optionsHeight + 'px' };

    return (
      <div
        style={selectorStyles}
        className={`${classNames &&
          classNames.autocompleteSelector &&
          classNames.autocompleteSelector}
      ...
      >
        {filteredOptions.map((option, index) => (
          <div
            key={index}
            style={optionsStyles}
            className={`${hoveredOptionIndex === index ? 'active' : ''} autocomplete-option`}
          ...
          >
            <span
              ...
              className={`${classNames && classNames.highlight && classNames.highlight}`}
            >
  ...
  handleKeyDownInput = options => e => {
    ...
    switch (e.key) {
      ...
      case 'Enter':
        hoveredOptionIndex !== null && this.handleClickOption(e, options[hoveredOptionIndex]);
        break;
  ...
  render() {
    const { onFocus, style = {}, classNames = {} } = this.props;
    const { value } = this.state;
    ...
    return (
      <div
        className={`${classNames &&
          classNames.autocompleteWrapper &&
          classNames.autocompleteWrapper} autocomplete-wrapper`}
      ...
      >
        <input
          ...
          style={inputStyle}
          className={`${classNames &&
            classNames.autocompleteInput &&
            classNames.autocompleteInput} autocomplete-input`}
